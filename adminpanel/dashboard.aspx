    ...
    <%--<script>
        ...
        $.getJSON(url, function (val) {
            ...
            for (index = 0; index < val.length; index++) {
                val[index].today = 0;
                val[index].thisweek = 0;
                val[index].thismonth = 0;
                val[index].lastweek = 0;
                val[index].lastmonth = 0;
            }
            
            for (index = 0; index < val.length; index++) {

                dateFld = Date.parseExact(val[index].MarkDate.replace('T', ' ').split('.')[0], "yyyy-MM-dd HH:mm:ss");

                if (dateFld.clearTime().between(Date.monday(), Date.next().sunday())) {

                    //thisweek[thisweek.length] = index;
                    //thismonth[thismonth.length] = index;

                    val[index].thisweek = 1;
                    val[index].thismonth = 1;

                    //continue;
                }

                if (Date.today().getMonth() == dateFld.getMonth()) {
                    //thismonth[thismonth.length] = index;
                    val[index].thismonth = 1;
                }

                if (dateFld.clearTime().between(Date.last().week().monday(), Date.last().sunday())) {
                    //lastweek[lastweek.length] = index;
                    val[index].lastweek = 1;
                }

                if (Date.today().getMonth() - 1 == dateFld.clearTime().getMonth()) {
                    //lastmonth[lastmonth.length] = index;
                    val[index].lastmonth = 1;

                }

                if (dateFld.clearTime().equals(Date.today().clearTime())) {

                    //today[today.length] = index;
                    //thisweek[thisweek.length] = index;
                    //thismonth[thismonth.length] = index;
                    ...
                    //alert(val[index].thimonth);
                    //continue;
                }
            }

            prompt("final", JSON.stringify(val));

            for (index = 0; index < val.length; index++) {
                if (val[index].today == 1) {
                alert(val[index].MarkDate);
                }
            }
        });
